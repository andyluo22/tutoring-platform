...
async def create_session(db: AsyncSession, sess_in: schemas.SessionCreate) -> models.Session:
    db_sess = models.Session(**sess_in.dict())
    db.add(db_sess)
    await db.commit()
...
async def create_booking(db: AsyncSession, book_in: schemas.BookingCreate) -> models.Booking:
    db_book = models.Booking(**book_in.dict())
    db.add(db_book)
    await db.commit()
...
async def create_session_signup(
    db: AsyncSession, signup_in: schemas.SessionSignupCreate
) -> models.SessionSignup:
    # note: invite_code + is_paid will be set in the router
    db_signup = models.SessionSignup(**signup_in.dict())
    db.add(db_signup)
    await db.commit()
    await db.refresh(db_signup)
    return db_signup

async def count_session_signups(
